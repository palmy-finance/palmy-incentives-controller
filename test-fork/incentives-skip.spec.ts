...
describe('Enable incentives in target assets', () => {
  ...
  */
  before(async () => {
    ...
    await (
      await selfDestructContractV3.destroyAndTransfer(OASYSLEND_SHORT_EXECUTOR, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
  ...
  it('Proposal should be executed', async () => {
    const impersonateExecutor = await ethers.provider.getSigner(OASYSLEND_SHORT_EXECUTOR);
  ...
  it('Check emission rate', async () => {
    ...
    const expectedClaimedRewards =
      BigNumber.from(expectedAccruedRewards).add(unclaimedRewardsBefore);

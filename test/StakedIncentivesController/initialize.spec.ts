...
makeSuite('IncentivesController initialize', (testEnv: TestEnv) => {
  it('Tries to call initialize second time, should be reverted', async () => {
    ...
    await expect(incentivesController.initialize(ZERO_ADDRESS, ZERO_ADDRESS)).to.be.reverted;
  });
  ...
  it('should be reverted if emissionManager is zero address', async () => {
    ...
    await expect(target.initialize(ZERO_ADDRESS, stakedToken.address)).to.be.revertedWith(
  ...
  it('should be reverted if rewardToken is zero address', async () => {
    ...
    await expect(target.initialize(incentivesController.address, ZERO_ADDRESS)).to.be.revertedWith(
      'INVALID_STAKED_TOKEN'
    );
  });
});
